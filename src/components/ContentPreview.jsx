...
import {
  ...
  Clock,
  Building,
} from "lucide-react"; // Lucide icons for a modern feel
...
export default function ContentPreview() {
  ...
  if (!selectedProject) {
    return (
      <div className="p-8">
        <Empty description="Loading or no project selected..." />
      </div>
    );
  }
  if (selectedUnits.length === 0) {
    return (
      <div className="p-8">
        <Card>
          <Empty
            description={`Select units from ${selectedProject.project_name} to generate content.`}
          />
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-8 bg-white p-2 rounded-lg">
      ...
      <div className="relative h-96 rounded-lg overflow-hidden shadow-lg">
        ...
        <div className="absolute bottom-0 left-0 p-8 text-white">
          ...
          <Paragraph className="text-white/90 text-lg max-w-2xl">
            A premium investment portfolio of {selectedUnits.length} units
            located in the heart of {dynamicData.area?.area_name_en}.
          </Paragraph>
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} sm={12} md={6}>
            <InfoCard
              ...
              value={dynamicData.area?.area_name_en}
              subValue={dynamicData.zone?.zone_name_en}
            />
          </Col>
          <Col xs={24} sm={12} md={6}>
            <InfoCard
          ...
          <Col xs={24} sm={12} md={6}>
            <InfoCard
              icon={<Home size={32} />}
              title="Availability"
              value={`${dynamicData.projectUnitsAvailable} / ${selectedProject.total_units}`}
          ...
          <Col xs={24} sm={12} md={6}>
            <InfoCard
              ...
              title="Price Range"
              value={formatCurrency(selectedProject.min_price)}
              subValue={`to ${formatCurrency(selectedProject.max_price)}`}
            />
          </Col>
        </Row>
      </div>
      ...
      {/* --- NEW CTA SECTION --- */}
      <div
        className="p-8 md:p-12 text-center text-white rounded-lg"
        style={{ background: "linear-gradient(135deg, #10b981, #2563eb)" }}
      >
        <Title level={2} className="!text-white">
          Secure your high-yield investment today
        </Title>
        <Paragraph className="text-white/80 max-w-3xl mx-auto">
          With flexible payment plans and guaranteed returns, this is a prime
          opportunity to grow your wealth through Dubai real estate. Our
            ...
            <Col xs={24} md={12}>
              ...
              <Space direction="vertical" size="middle" className="w-full">
                <div className="flex items-center gap-3">
                  ...
                  <Text>Dubai Marina, Dubai, UAE</Text>
                </div>
                <div className="flex items-center gap-3">
                  <Phone size={20} className="text-gray-500" />
                  <Text>+971 4 XXX XXXX</Text>
                </div>
                <div className="flex items-center gap-3">
                  <Mail size={20} className="text-gray-500" />
                  <Text>sales@thinkrealty.ae</Text>
                </div>
                <div className="flex items-center gap-3">
                  <Clock size={20} className="text-gray-500" />
                  <Text>Open 7 days a week, 9 AM - 9 PM</Text>
                </div>
              </Space>
            </Col>
            <Col xs={24} md={12}>
              ...
              <Space direction="vertical" size="middle" className="w-full">
                <div className="flex items-center gap-3">
                  ...
                  <Text>Developer: {selectedProject.developer}</Text>
                </div>
                <div className="flex items-center gap-3">
                  <Clock size={20} className="text-gray-500" />
                  <Text>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Text>Total Units: {selectedProject.total_units}</Text>
                </div>
                <div className="flex items-center gap-3">
                  <TrendingUp size={20} className="text-gray-500" />
                  <Text>
                    Handover:{" "}
                    {getHandoverQuarter(selectedProject.completion_date)}
                  </Text>
                </div>
              </Space>
            </Col>
