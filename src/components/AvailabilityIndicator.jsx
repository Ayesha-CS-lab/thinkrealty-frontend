...
export default function AvailabilityIndicator() {
  ...
  const projectUnits = allUnits.filter(
    (u) => u.project_id === selectedProject.project_id
  );
  const availableCount = projectUnits.filter(
  ...
  return (
    <Card title="Project Availability Status">
      ...
      <Progress
        percent={parseFloat(availablePercent.toFixed(1))}
        strokeColor={availablePercent < 20 ? "#faad14" : "#52c41a"}
      />
            ...
            {Object.entries(timers).map(([unitId, time]) => (
              ...
              >
                <Text>
                  Unit{" "}
                  <Text strong>
                    {allUnits.find((u) => u.unit_id == unitId)?.unit_number}
                  </Text>
                </Text>
