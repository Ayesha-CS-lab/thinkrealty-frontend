...
// --- Helper Functions to Simulate Premiums ---
const BASE_PRICE_PERCENT = 0.95; // Base price is 95% of total
const FLOOR_PREMIUM_RATE = 500; // AED 500 premium per floor level
const BALCONY_PREMIUM_PERCENT = 0.08; // 8% premium on base price
const PARKING_FEE = 15000; // Fixed fee per parking spot
...
export default function PricingBreakdown({ selectedUnits }) {
  const totals = useMemo(() => {
    ...
    return selectedUnits.reduce(
      ...
      { basePrice: 0, floorPremium: 0, balconyPremium: 0, parkingFee: 0, total: 0 }
    );
  }, [selectedUnits]);
    ...
    <Card title="Pricing Breakdown" size="small">
       <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-blue-50 p-3 rounded-lg text-center">
            <Text type="secondary" className="text-xs">Total Investment</Text>
            <Title level={5} className="!mt-1 !mb-0 text-blue-600">{formatCurrency(totals.total)}</Title>
        </div>
         <div className="bg-green-50 p-3 rounded-lg text-center">
            <Text type="secondary" className="text-xs">Units Selected</Text>
            <Title level={5} className="!mt-1 !mb-0 text-green-600">{selectedUnits.length}</Title>
        </div>
       </div>
        ...
        <div className="space-y-3 mt-2">
            <div className="flex justify-between items-center text-sm">
                ...
                <Text className="text-green-600 flex items-center gap-1"><RiseOutlined /> {formatTinyCurrency(totals.basePrice)}</Text>
            </div>
            <div className="flex justify-between items-center text-sm">
                <Text type="secondary">Floor level premiums</Text>
                 <Text className="text-green-600 flex items-center gap-1"><RiseOutlined /> {formatTinyCurrency(totals.floorPremium)}</Text>
            </div>
            <div className="flex justify-between items-center text-sm">
                <Text type="secondary">Balcony premiums (+8%)</Text>
                 <Text className="text-red-500">{formatCurrency(totals.balconyPremium)}</Text>
            </div>
            <div className="flex justify-between items-center text-sm">
                <Text type="secondary">Parking fees (AED 15,000 per unit)</Text>
                 <Text className="text-red-500">{formatCurrency(totals.parkingFee)}</Text>
            </div>
        </div>

      <Divider className="my-3" />
       <div className="flex justify-between items-center">
        <Title level={5} className="!mb-0">Total Investment</Title>
        <Title level={4} className="!mb-0 text-blue-600">{formatCurrency(totals.total)}</Title>
      </div>
    </Card>
  );
}