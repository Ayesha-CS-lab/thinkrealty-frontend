...
export default function PricingCalculator() {
  ...
  const { breakdown } = useSelector(
    (state) => state.landingPage.pricingCalculations
  );
  ...
  if (!breakdown || breakdown.length === 0) {
    return (
      <Card title="Pricing Calculator">
        <Text type="secondary">
          Select units to see a detailed price breakdown.
        </Text>
      </Card>
    );
  }
  ...
  return (
    <Card
      title={
        <>
          <DollarOutlined /> Pricing Calculator
        </>
      }
    >
      <div className="space-y-2">
        {breakdown.map((item, index) => {
          ...
          return (
            ...
            >
              <Text type={item.isTotal ? "default" : "secondary"}>
                {item.label}
              </Text>
              <Text
                strong={item.isBold}
              ...
              >
                ...
                {item.isFuture && (
                  <Tooltip title="Estimated value based on market appreciation">
